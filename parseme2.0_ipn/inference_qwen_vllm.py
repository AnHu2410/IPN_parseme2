...
class QwenInferencer:
	def __init__(self, data_path, output_path, prompt, model_dir):
		...
		self.llm = LLM(model="../Qwen3-32B-INT8", enable_lora=True, max_model_len=1500, gpu_memory_utilization=0.85, enforce_eager=True)
		self.data_path = data_path	
		self.data = self.read_data_cupt()
		self.output_path = output_path
		self.prompt = prompt
		self.sampling_params = SamplingParams(temperature=0.6, top_p=0.95, top_k=20, max_tokens=300)
